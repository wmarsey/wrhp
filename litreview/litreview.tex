...
\begin{document}
	...
	%% \end{framed}
        ...
        \textbf{For this study we will assume that the 'final' or
          'target' version is fully trusted, or that it's quality does
          not need evaluating.} This way we can leave issues of trust
        regarding the article to one side, and concentrate on the
        various contributors, and their edits. It is the opinion in
        this study that the reputation, or quality, of the article
        itself is not important here. Our intention is to evaluate an
        individual's weighted stake in an article - whether that
        article is of good or bad quality is something of a different
        issue.

        That said, some of the methods used to measure quality are
        directly applicable here. In particular we look at the studies
        by 
