...
\begin{figure}[p]
  ...
  \begin{tikzpicture}[
    ...
    \node (h) [block2,right=1cm of g] {We can use\HS\HS to eat spaghetti.};
    ...
    \node (l) [block2,below =1cm of h] {We can use\HS\HS to eat spaghetti.};

    \node (m) [block2,below= 1cm of i] {$ed_{links} = 3$};

    \node (n) [block2,below =1cm of k] {$ed_{normal} = 2$};
