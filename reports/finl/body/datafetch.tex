...
\begin{algorithmic}
  $revs \gets database.fetchIDsofpage()$\\
...
\begin{algorithmic}
  $revs \gets database.fetchIDsofpage()$\\
  $target \gets database.gettext(revs[-1])$
...
\begin{algorithmic}
  $regexes \gets $\{\\
    ...
    ...\\
  \}\Comment{Regexes that recognise single Wikimarkup tags}\\
  $reggroups \gets $\{\\
    'maths':(regexes['math1'], regexes['math2']),\\
    ...\\
  \}\Comment{Group of regexes by 'species'}
  $distances \gets {}$
  \For {$key, reg \in reggroups$}
