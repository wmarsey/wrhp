...
\begin{frame}
  ...
  \onslide<2>{
    ...
    Though we are inevitably be waiting on the final process (the normal
    text), we noticed a sizable increase in speed:
...
\note{Small articles are fast, and even large articles were small
  ...
  \begin{frame}{Possible optimisations}
    \centering
    \only<1>{ 
...
\section{Iplementation}
\begin{frame}

  \begin{figure}
  \centering
  \begin{tikzpicture}[align=center,node distance=0.4cm,font=\tiny]
