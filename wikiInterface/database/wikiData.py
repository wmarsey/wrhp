...
class WikiDatabase:
    ...
    def cleanup(self):
        sql = "DELETE FROM " + self.weighttable + " JOIN " + self.revisiontable + " USING (revid, domain) WHERE pageid NOT IN (SELECT pageid, domain FROM " + self.fetchedtable + "); DELETE FROM " + self.trajectorytable + " AS t JOIN " + self.revisiontable + " AS r USING (revid, domain) WHERE pageid, domain NOT IN (SELECT pageid, language FROM " + self.fetchedtable + "); DELETE FROM " + self.contenttable + " AS t JOIN " + self.revisiontable + " AS r USING (revid, domain) WHERE pageid, domain NOT IN (SELECT pageid, language FROM " + self.fetchedtable + "); DELETE FROM " + self.revisiontable + " WHERE pageid, domain NOT IN (SELECT pageid, language FROM " + self.fetchedtable + ");"
        if(self._execute(sql,())):
            result = self._crsrsanity()
            if result:
                return result
    ...
    def clear(self):
        sql = "DELETE FROM " + self.revisiontable + "; DELETE FROM " + self.contenttable + "; DELETE FROM " + self.trajectorytable + "; DELETE FROM " + self.weighttable + "; DELETE FROM " + self.fetchedtable + ";"
        if(self._execute(sql,())):
            result = self._crsrsanity()
            if result:
                return result
    ...
    def __init__(self):
        with open(BASEPATH + '/dbpas','r') as pasfil:
